...
**다양한 피지컬 AI 융합 수업을 펼치는 장** — 서울특별시교육청 AI정보교육연구회(AI·피지컬컴퓨팅융합교육연구회) 학교자율시간 프로그램 개발용 플랫폼
...
- **교육청 자료실** — 『피지컬 AI 원리를 활용한 문제해결 프로젝트 자료집』 9개 챕터 JSON(`src/data/curriculum/`) 열람 + **AI 수업 재구성**(우리 교구·학급 기준 예시 프로그램 + 차시 계획)
- **언플러그드** — 『놀이로 경험하는 디지털 세상 언플러그드 학습가이드 지도자료』 16개 활동 JSON(`src/data/unplugged/`) + 이어지는 피지컬 AI 활동 추천
...
src/data/curriculum/*.json  교육청 자료집 챕터(9)
src/data/unplugged/*.json   언플러그드 지도자료(16 활동)
src/lib/ai.js               Gemini 호출 · 프롬프트 · 검증/수정 체이닝
```

## 출처 및 유의

- 교육 자료: 서울특별시교육청 『피지컬 AI 원리를 활용한 문제해결 프로젝트 자료집』, 『놀이로 경험하는 디지털 세상 언플러그드 학습가이드 지도자료』(담당 장학사 배포 자료를 JSON으로 구조화, 원문 표기 유지).
