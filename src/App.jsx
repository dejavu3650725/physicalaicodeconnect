...
import { Bot, Home as HomeIcon, Cpu, Plug, Compass, Menu, X, FolderOpen, LogIn, LogOut } from 'lucide-react';
...
import { isFirebaseEnabled, useAuth, signIn, signOut } from './lib/firebase.js';
...
export default function App() {
  ...
  const nav = fb ? [...NAV, { to: '/mine', label: '내 설계', icon: FolderOpen }] : NAV;
  ...
  else if (route.segs[0] === 'view' || route.segs[0] === 'd') page = <View route={route} />;
  else if (route.segs[0] === 'mine') page = <MyDesigns route={route} />;
      ...
      <nav className="topbar sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center gap-4">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {fb && (user
              ? <button onClick={() => signOut()} className="flex items-center gap-2 rounded-xl pl-1 pr-2.5 py-1 hover:bg-white/10 text-xs font-bold text-slate-200" title="로그아웃">{user.photoURL ? <img src={user.photoURL} alt="" className="w-7 h-7 rounded-full" referrerPolicy="no-referrer" /> : <span className="w-7 h-7 rounded-full bg-lime-400 text-[#0b1220] grid place-items-center font-black">{(user.displayName || '?')[0]}</span>}<span className="hidden sm:inline max-w-[7rem] truncate">{user.displayName || user.email}</span><LogOut className="w-3.5 h-3.5 text-slate-400" /></button>
              : <button onClick={() => signIn().catch(() => {})} className="flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-bold text-slate-200 hover:bg-white/10 whitespace-nowrap"><LogIn className="w-4 h-4" /> 로그인</button>)}
