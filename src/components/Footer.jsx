...
const PRIVACY = {
  ...
  articles: [
    { no: 1, title: '개인정보의 처리 목적 및 항목', body: [
      `서비스는 회원 가입, 로그인, 이름·연락처 입력 등 개인정보를 요구하는 절차가 없으며, 운영자가 이용자의 개인정보를 별도로 수집·저장하는 서버 데이터베이스를 운영하지 않습니다.`,
      `이용자가 아이디어 입력란에 스스로 기재한 내용은 설계 결과 생성을 위해서만 처리되며, 운영자는 여기에 개인정보를 포함하지 않도록 안내합니다.`,
    ] },
    { no: 2, title: '브라우저 저장 정보(localStorage)', body: [
    ...
    { no: 3, title: '외부 인공지능 서비스로의 전송', body: [
    ...
    { no: 5, title: '개인정보의 보유 및 파기', body: [
      `운영자는 개인정보를 별도로 보유하지 않으므로 파기 대상 정보가 없습니다. 이용자 기기에 저장된 브라우저 정보는 이용자가 직접 삭제할 수 있습니다.`,
    ] },
    { no: 6, title: '이용자의 권리', body: [
