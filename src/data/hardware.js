// ============================================================
// 하드웨어 프로필 — 코드 커넥트/튜토리얼/자료실이 공통으로 참조
// ============================================================
export const HARDWARE = [
  {
    ...
    curriculum: ['hamster-basic', 'hamster-intermediate'],
    tutorial: 'hamster',
  },
  {
    ...
    aiHow: '① microbit.org CreateAI(동작 데이터 학습 → 제스처 인식 모델을 마이크로비트에 탑재) ② 엔트리와 마이크로비트를 연결해 엔트리 AI 블록(이미지·음성)과 결합(교육청 초급 자료 방식) ③ 티처블 머신 + 블루투스(boundaryX, 교육청 중·고급 자료 방식).',
    ideas: ['걸음 수를 세는 스마트워치(만보기)', '체온·주변 온도 알림 밴드', '흔들면 가위바위보', '친구와 라디오로 비밀 메시지', '소리가 커지면 경고하는 소음 측정기', '나침반 방향 안내 시계'],
    curriculum: ['microbit-basic', 'microbit-intermediate', 'microbit-advanced'],
    tutorial: 'microbit',
  },
  {
    ...
    connection: '드론↔조종기 페어링 → 조종기 USB(CH340) → 엔트리 하드웨어 연결 프로그램(제조사 안내 모듈 선택)',
    sensors: ['바닥 거리 센서(ToF, 유효 2m)', '정면 거리 센서', '기압 고도(해발고도)', '자세(Roll/Pitch/Yaw)·가속도', '옵티컬 플로우 위치(X/Y)', '조종기 버튼·조이스틱 입력', '배터리'],
    actuators: ['이륙/착륙/정지', '방향별 거리 이동(m)·회전(도)', 'Roll/Pitch/Yaw/Throttle 제어(%)', '드론 LED(색·모드)', '조종기/드론 버저'],
    ...
    curriculum: [],
    tutorial: 'tory',
    caution: '토리드론의 엔트리 블록 이름은 같은 계열(바이로봇 코딩드론) 블록을 기준으로 표시합니다. 실제 화면과 조금 다를 수 있으니 교재/제조사 안내로 확인하세요.',
  },
  {
    ...
    aiHow: '교육청 자료 방식 — ① "AI 이거다"(PC 카메라 이미지/자세/얼굴 분류 모델을 SPIKE 앱 확장 블록으로 연결) ② HuskyLens AI 카메라(색·얼굴·물체 추적)를 허브에 연결.',
    ideas: ['쓰레기 종류를 인식해 분류하는 수거 로봇', '손 모양으로 조종하는 로봇팔', '검은 선을 따라가는 스마트 모빌리티', '장애물 앞에서 멈추는 자율주행차', '표정을 읽는 반려 로봇', '마스크 인식 출입 차단기'],
    curriculum: ['spike-basic', 'spike-intermediate', 'spike-advanced-social', 'spike-advanced-huskylens'],
    tutorial: 'spike',
  },
];
...
export const LEVELS = [
  { key: 'basic', name: '기초', emoji: '🌱', desc: '움직임·출력 중심의 순차 구조', full: '🌱 기초 (기본 제어)' },
  { key: 'standard', name: '기본', emoji: '🚀', desc: '센서 값에 따라 판단하는 반복·조건 구조', full: '🚀 기본 (센서와 데이터)' },
  { key: 'advanced', name: '심화', emoji: '🔥', desc: 'AI 인식·학습 모델과 결합한 융합 프로젝트', full: '🔥 심화 (인공지능 융합)' },
];
