...
export const HARDWARE = [
  ...
  },
  {
    ...
    connection: '드론 배터리 → 페어링(P 3초) → 조종기 USB 연결 → 사이드 LED 깜빡임(Link 모드) 확인 → Entry_Alux › 하드웨어 › 연결 프로그램 열기 › 토리드론 (또는 엔트리 하드웨어 프로그램에서 바이로봇 배틀 드론)',
    specs: { size: '135×135×42 mm', weight: '38 g(배터리 포함)', battery: '리튬폴리머 3.7V 300mAh 20C', flight: '약 5~6분', charge: '약 40분', radio: '2.4GHz RF 양방향 · 최대 100m(권장 50m)', sensors: '6축 관성센서 + 비전 센서(X,Y,Z)', controllerBattery: 'AAA×2(별도)' },
    controller: [
      { key: 'L 3초', what: '자동 이륙 / 착륙' }, { key: 'P 3초', what: '페어링' }, { key: 'S 3초', what: '자세 센서 캘리브레이션(페어링 상태) / 레버 캘리브레이션(페어링 끊긴 상태)' },
      { key: 'B 2초', what: '리턴 홈(이륙 지점으로 직선 복귀)' }, { key: 'A 3초 / 짧게', what: '헤드리스 모드 설정·해제 / 비행 중 Heading Reset' }, { key: 'R 짧게 / 누르고 레버', what: 'LED 색 변경(흰→무지개→빨강→초록→파랑→아쿠아→핑크→노랑) / 360도 플립' },
      { key: 'Trim ▲▼◀▶', what: '흐름 보정(흐르는 반대 방향 버튼) · 착륙 상태에서 3초 = 조종 모드1/2 변경' }, { key: '전원 짧게', what: 'USB 연결 시 조종 모드(점등) ↔ Link 모드(깜빡임 · 코딩 가능)' }, { key: '왼쪽 레버 ↓ + 좌측 상단', what: '비상정지(공중에서는 추락 주의)' },
    ],
    ...
    limits: '카메라 없음(영상 인식은 PC 카메라 + 엔트리 AI 블록). 배터리 3.7V 300mAh — 비행 5~6분/충전 40분이라 비행 시간을 아껴야 한다(코드 논리는 LED·부저로 지상 테스트 후 비행). PC 명령은 조종기가 Link 모드(사이드 LED 깜빡임)일 때만 받는다. 실내 비행 시 무늬 있는 바닥, 1.5m 이하 고도, 안전 거리 필수.',
    ...
    tutorial: 'tory',
    caution: '블록 명칭은 잇플 교재 『엔트리 인공지능과 함께하는 토리드론』 화면 기준. 엔트리로 제어하려면 조종기가 USB 연결 + Link 모드(사이드 LED 깜빡임)여야 해요 — 점등(조종 모드)이면 전원 버튼을 짧게 눌러 전환. 비행 시간이 5~6분이니 LED·부저로 지상 테스트 후 비행하세요.',
  },
  {
